export type BridgeApiClientOptions = { baseUrl: string; fetch: typeof globalThis.fetch };
...
export class BridgeApiClient {
  ...
  constructor(options: BridgeApiClientOptions) { this.#baseUrl = options.baseUrl.replace(/\/$/, ""); this.#fetch = options.fetch; }
  async #json<T>(path: string, init?: RequestInit): Promise<T> { const response = await this.#fetch(`${this.#baseUrl}${path}`, { ...init, headers: { accept: "application/json", ...(init?.headers ?? {}) } }); if (!response.ok) throw new Error(`POWERCHAIN_BRIDGE_HTTP_${response.status}`); return await response.json() as T; }
  ...
  quote<T = Record<string, unknown>>(input: BridgeQuoteRequest, init?: RequestInit) { return this.#json<T>("/api/v1/bridge/quote", { ...init, method: "POST", headers: { "content-type": "application/json", ...(init?.headers ?? {}) }, body: JSON.stringify(input) }); }
  createTransfer<T = Record<string, unknown>>(input: BridgeTransferCreateRequest, idempotencyKey: string, init?: RequestInit) { return this.#json<T>("/api/v1/bridge/transfers", { ...init, method: "POST", headers: { "content-type": "application/json", "idempotency-key": idempotencyKey, ...(init?.headers ?? {}) }, body: JSON.stringify(input) }); }

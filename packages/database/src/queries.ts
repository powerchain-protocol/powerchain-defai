...
export async function persistWalletBalanceSnapshots(rows:readonly WalletBalanceSnapshotInput[]){
  ...
  return getPrismaClient().walletBalanceSnapshot.createMany({data:rows.map((row)=>({id:randomUUID(),...row}))});
}
export async function recentSwapExecutions(input:{wallet:string;limit?:number}){
  const limit=Math.max(1,Math.min(100,input.limit??20));
  return getPrismaClient().swapExecution.findMany({where:{payer:input.wallet},orderBy:{createdAt:"desc"},take:limit});
}

...
const security=read("apps/backend/src/services/security.ts"); for(const token of ["API_KEY_HEADER","X-Api-Key","API_KEY_MIN_LENGTH","API_KEY_MAX_LENGTH","authorizeApiKey","API_KEY_REQUIRED","API_KEY_INVALID","API_KEY_NOT_CONFIGURED"]) if(!security.includes(token)) throw new Error(`API_KEY_SECURITY_MISSING:${token}`);
const runtime=read("scripts/env-runtime-check.mjs"); for(const token of ["24-256 character API key","duplicate valid keys"]) if(!runtime.includes(token)) throw new Error(`API_KEY_RUNTIME_VALIDATION_MISSING:${token}`);
const smoke=read("scripts/production-smoke.mjs"); for(const token of ["POWERCHAIN_SMOKE_API_KEY","x-api-key","API key required"]) if(!smoke.includes(token)) throw new Error(`API_KEY_SMOKE_SUPPORT_MISSING:${token}`);

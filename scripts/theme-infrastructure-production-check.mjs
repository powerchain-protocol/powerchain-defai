...
for (const token of ["data-theme=\"light\"", "ThemeToggle", "suppressHydrationWarning"]) must(layout.includes(token), `layout missing ${token}`);
...
for (const token of ["Open navigation menu", "Close navigation menu", "Escape", "event.key !== \"Tab\"", "document.body.style.overflow", "/claim", "/integrations"]) must(mobileMenu.includes(token), `mobile navigation missing ${token}`);
for (const token of ["Loading bridge workspace", "animate-pulse"]) must(appLoading.includes(token), `root loading state missing ${token}`);
...
must(layout.includes("pc-glass"), "application header must use the controlled cinematic glass primitive");
...
must(vercel.$schema === "https://openapi.vercel.sh/vercel.json", "Vercel schema missing");

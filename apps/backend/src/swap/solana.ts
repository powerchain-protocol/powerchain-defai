...
import { resolveJupiterRequestConfig, type JupiterRequestOverride } from "../integrations/dex/jupiter";
...
export async function createJupiterSwapOrder(input: { payer: string; inputMint: string; outputMint: string; amountBaseUnits: string; slippageBps: number }, override?: JupiterRequestOverride): Promise<SolanaSwapOrder> {
  ...
  const jupiter = resolveJupiterRequestConfig(override);
  const url = new URL(`${jupiter.apiUrl}/order`);
  ...
  const response = await fetchIntegrationJson<JupiterOrderResponse>(url.toString(), { headers: { "x-api-key": jupiter.apiKey } }, 10_000);
...
export async function executeJupiterSwap(input: { payer: string; signedTransaction: string; requestId: string; lastValidBlockHeight?: number | null; inputMint: string; outputMint: string; amountBaseUnits: string; slippageBps: number; minimumOutputBaseUnits?: string | null }, override?: JupiterRequestOverride) {
  wallet(input.payer);
  ...
  const result = await fetchIntegrationJson<{ status?: string; code?: number; signature?: string; error?: string; totalInputAmount?: string; totalOutputAmount?: string }>(`${jupiter.apiUrl}/execute`, { method: "POST", headers: { "x-api-key": jupiter.apiKey, "content-type": "application/json" }, body: JSON.stringify(body) }, 15_000);

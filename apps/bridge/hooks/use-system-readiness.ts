...
import { fetchSystemReadiness } from "@/backend/system-readiness-client";
import { providerErrorMessage } from "@/common/provider-errors";
import type { SystemReadinessPayload } from "@/types/system-readiness";

export function useSystemReadiness(refreshMs = 30_000) {
  const [data, setData] = useState<SystemReadinessPayload | undefined>();
  ...
  const refresh = useCallback(async () => {
    ...
    setRefreshing(true);
    try {
      const result = await fetchSystemReadiness(next.signal);
      if (current !== generation.current) return;
      setData(result);
      setError(undefined);
    } catch (reason) {
      if (next.signal.aborted || current !== generation.current) return;
      setError(providerErrorMessage(reason, "System readiness unavailable"));
    } finally {

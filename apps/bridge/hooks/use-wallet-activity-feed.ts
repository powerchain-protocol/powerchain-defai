...
export function useWalletActivityFeed(solanaAddress?: string | null, suiAddress?: string | null, limit = 25) {
  ...
  const load = useCallback(async (nextCursor: string | null, append: boolean) => {
    ...
    try {
      ...
      const response = await apiFetch(`/api/v1/wallet/activity?${params}`, { cache: "no-store", signal: abort.signal });
      const payload = await response.json();
      if (current !== generation.current) return;
      if (!response.ok && response.status !== 503) throw new Error(payload?.message || `HTTP ${response.status}`);
      setPages((old) => append ? [...old, payload] : [payload]);
      ...
      if (current === generation.current) setError(abort.signal.aborted ? "Wallet activity request timed out" : cause instanceof Error ? cause.message : "Wallet activity unavailable");
    } finally {

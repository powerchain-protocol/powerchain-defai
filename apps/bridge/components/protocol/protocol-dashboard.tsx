...
function ProgramCard({ item, refreshing, stale, onRefresh }: { item: ProgramRuntimeItem; refreshing: boolean; stale: boolean; onRefresh: () => void }) {
  return <article className="pc-panel rounded-[24px] p-5 sm:p-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      ...
      <div className="flex shrink-0 flex-col gap-2 sm:items-end">
        ...
        <button type="button" onClick={onRefresh} disabled={refreshing} className="min-h-9 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 disabled:opacity-50 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-200 dark:hover:bg-white/[0.08]">{refreshing ? "Verifying…" : "Verify program"}</button>
      </div>
    </div>
...
export function ProtocolDashboard() {
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="inline-flex w-full rounded-xl border border-slate-200 bg-slate-50 p-1 dark:border-white/10 dark:bg-white/[0.03] sm:w-auto" role="group" aria-label="Program scope">{(["all","core","optional"] as const).map((value)=><button key={value} type="button" onClick={()=>setScope(value)} aria-pressed={scope===value} className={`min-h-9 flex-1 rounded-lg px-3 text-xs font-semibold capitalize transition sm:flex-none ${scope===value ? "bg-white text-slate-950 shadow-sm dark:bg-white/10 dark:text-white" : "text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200"}`}>{value}</button>)}</div><p className="text-xs text-slate-500">{visiblePrograms.length} of {programs.length || 4} programs shown</p></div>
    ...
    {runtime.error ? <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-200">Program verification unavailable: {runtime.error}</div> : null}

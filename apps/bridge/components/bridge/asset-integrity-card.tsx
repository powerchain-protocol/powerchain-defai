...
function ChainState({ name, ok, detail }: { name: string; ok: boolean | undefined; detail: string | null | undefined }) {
  return <div className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 px-3 py-2.5 dark:border-slate-800">
    ...
    <span className={`rounded-full px-2 py-1 text-xs font-medium ${ok === undefined ? "bg-slate-100 text-slate-600 dark:bg-slate-900 dark:text-slate-300" : ok ? "bg-[#f1f4f2] text-[#294a3b] dark:bg-[#09110e]/60 dark:text-[#d0dcd6]" : "bg-amber-50 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300"}`}>
      {ok === undefined ? "Pending" : ok ? "Verified" : "Attention"}
    </span>
  </div>;
}

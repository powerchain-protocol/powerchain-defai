...
export function TransferDeepLink({ transferId, className = "" }: { transferId: string; className?: string }) {
  ...
  return <div className={`flex flex-wrap items-center gap-2 ${className}`}>
    <a href={href} className="inline-flex min-h-9 items-center rounded-lg bg-blue-600 px-3 text-xs font-semibold text-white hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2">Open transfer</a>
    <button type="button" onClick={() => void copy()} aria-label="Copy transfer status link" className="min-h-9 rounded-lg border border-slate-200 px-3 text-xs font-semibold text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:border-slate-700 dark:text-slate-200">{copyState === "copied" ? "Link copied" : copyState === "error" ? "Copy failed" : "Copy link"}</button>

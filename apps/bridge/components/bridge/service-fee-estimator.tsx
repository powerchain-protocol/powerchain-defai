  ...
  return (
    <section className="rounded-[20px] border border-slate-200 bg-white p-4 shadow-sm sm:p-6 dark:border-slate-800 dark:bg-slate-950" aria-labelledby="fee-estimator-title">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#294a3b] dark:text-[#adc0b6]">Service fee</p>
        ...
        <span className="mt-2 inline-flex w-fit rounded-full border border-[#d4ddd8] bg-[#f1f4f2] px-2.5 py-1 text-xs font-semibold text-[#294a3b] sm:mt-0 dark:border-[#29483c] dark:bg-[#09110e]/60 dark:text-[#d0dcd6]">1:1 principal</span>
      </div>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_180px_180px_auto] lg:items-end">
        <label className="block">
          ...
          <input value={routeId} onChange={(e) => setRouteId(e.target.value)} placeholder="Canonical route ID" autoComplete="off" className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-[#35584a] focus:ring-2 focus:ring-[#35584a]/20 dark:border-slate-700 dark:bg-slate-900 dark:text-white" />
        </label>
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-slate-700 dark:text-slate-300">Source chain</span>
          <select value={chain} onChange={(e) => setChain(e.target.value as Chain)} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm font-medium text-slate-950 outline-none focus:border-[#35584a] focus:ring-2 focus:ring-[#35584a]/20 dark:border-slate-700 dark:bg-slate-900 dark:text-white">
        ...
        <label className="block">
          ...
          <input inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm font-semibold tabular-nums text-slate-950 outline-none focus:border-[#35584a] focus:ring-2 focus:ring-[#35584a]/20 dark:border-slate-700 dark:bg-slate-900 dark:text-white" />
        </label>
        <button type="button" disabled={!valid || loading} onClick={estimate} className="h-11 rounded-xl bg-[#0b1511] px-5 text-sm font-semibold text-white transition hover:bg-[#102019] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#35584a] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-45 dark:bg-[#dfe7e3] dark:hover:bg-white">{loading ? "Calculating…" : "Calculate fee"}</button>
      </div>
...
function Metric({ label, value, hint, emphasize = false }: { label: string; value: string; hint: string; emphasize?: boolean }) {
  return (
    <div className={`rounded-2xl border p-4 ${emphasize ? "border-[#d4ddd8] bg-[#f1f4f2]/70 dark:border-[#29483c] dark:bg-[#09110e]/45" : "border-slate-200 bg-slate-50/70 dark:border-slate-800 dark:bg-slate-900/60"}`}>

    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/45 sm:items-center sm:p-6" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descId} className="max-h-[90dvh] w-full overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl dark:bg-slate-950 sm:max-w-lg sm:rounded-3xl sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div><div className="text-xs font-semibold uppercase tracking-[0.16em] text-[#294a3b]">Final review</div><h2 id={titleId} className="mt-1 text-lg font-semibold text-slate-950 dark:text-white">{title}</h2><p id={descId} className="mt-1 text-sm text-slate-600 dark:text-slate-400">{description}</p></div>
          <button ref={closeRef} type="button" onClick={onClose} className="min-h-10 rounded-xl border border-slate-200 px-3 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#35584a] dark:border-slate-700" aria-label="Close review">Close</button>
        </div>
        ...
        <div className="mt-5 grid gap-2 sm:grid-cols-2"><button type="button" onClick={onClose} className="min-h-11 rounded-xl border border-slate-200 px-4 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#35584a] dark:border-slate-700">Cancel</button><ActionStateButton onAction={onConfirm} pendingLabel={pendingLabel} className="bg-[#0b1511] text-white">{confirmLabel}</ActionStateButton></div>
      </section>
    </div>
  );
}

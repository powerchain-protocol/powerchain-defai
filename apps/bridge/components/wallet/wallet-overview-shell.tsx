...
function pill(status: string) {
  if (status === "ELIGIBLE" || status === "COMPLETED" || status === "FINALIZED") return "border-[#d4ddd8] bg-[#f1f4f2] text-[#214233] dark:border-[#29483c]/50 dark:bg-[#09110e]/40 dark:text-[#e0e8e4]";
...
export function WalletOverviewShell({ solanaAddress, suiAddress }: { solanaAddress?: string | null; suiAddress?: string | null }) {
  ...
  return <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8" aria-labelledby="wallet-title">
    <header className="flex flex-col gap-4 border-b border-slate-200 pb-5 dark:border-slate-800 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#294a3b]">PowerChain Wallet</p><h1 id="wallet-title" className="mt-1 text-2xl font-semibold tracking-tight text-slate-950 dark:text-white sm:text-3xl">PWRC & wPWRC</h1><p className="mt-2 max-w-2xl text-sm text-slate-600 dark:text-slate-400">Balances, claim readiness and cross-chain activity from Solana and Sui. Bridge accounting remains based on persisted finality and reconciliation evidence.</p></div>
      <div className="flex flex-wrap gap-2"><Link href="/bridge" className="min-h-10 rounded-xl bg-[#0b1511] px-4 py-2.5 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#35584a]">Bridge assets</Link><Link href="/assets" className="min-h-10 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#35584a] dark:border-slate-700 dark:text-slate-100">View assets</Link></div>
    </header>
    ...
    <section className="mt-5 grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
        <div className="flex items-start justify-between gap-4"><div><h2 className="font-semibold">PWRC ↔ wPWRC bridge</h2><p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Canonical PWRC on Solana and its 1:1 bridged representation on Sui.</p></div><span className="rounded-full border border-[#d4ddd8] bg-[#f1f4f2] px-2.5 py-1 text-xs font-semibold text-[#294a3b] dark:border-[#29483c] dark:bg-[#09110e]/50 dark:text-[#e0e8e4]">Wormhole NTT</span></div>
        ...
        <div className="mt-4 flex flex-wrap gap-2"><Link href="/bridge" className="rounded-lg bg-[#173b2d] px-3.5 py-2 text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#35584a]">Open bridge</Link><Link href="/fees" className="rounded-lg border border-slate-200 px-3.5 py-2 text-sm font-semibold dark:border-slate-700">View fees</Link></div>
      </div>

...
type ChainFilter = "ALL" | "SOLANA" | "SUI";
...
export function CrossChainActivityList({ activity, loading, error, hasNextPage, onLoadMore, onRefresh }: { activity: Activity[]; loading?: boolean; error?: string | null; hasNextPage?: boolean; onLoadMore?: () => void; onRefresh?: () => void }) {
  ...
  return <><section className="overflow-hidden rounded-[var(--pc-radius-card)] border border-slate-200 bg-white shadow-[0_14px_36px_rgba(7,16,13,.05)] dark:border-white/10 dark:bg-[#0a100d]" aria-labelledby="cross-chain-activity-title">
    <div className="flex flex-col gap-3 border-b border-slate-200 p-4 dark:border-white/8 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center"><Tabs value={chain} onValueChange={setChain} items={CHAIN_FILTERS} label="Filter activity by chain" className="min-w-[250px]" />{onRefresh?<Button size="sm" onClick={onRefresh} loading={loading} loadingLabel="Refreshing…">Refresh</Button>:null}</div>
    </div>
    {error?<div role="alert" className="p-4 text-sm text-rose-700 dark:text-rose-300">Cross-chain activity is temporarily unavailable. Refresh to try again.</div>:filtered.length?<div className="divide-y divide-slate-100 dark:divide-white/8">{filtered.map((row,index)=><ActivityRow key={`${activityId(row)}-${index}`} row={row} onOpen={()=>setSelected(toDetail(row))}/>)}</div>:<div className="p-8 text-center"><div className="text-sm font-semibold">No activity found</div><p className="mt-1 text-sm text-slate-500">Finalized Solana or Sui activity will appear here when indexed.</p></div>}
    {hasNextPage&&onLoadMore?<div className="border-t border-slate-200 p-3 text-center dark:border-white/8"><Button size="sm" disabled={loading} onClick={onLoadMore} loading={loading} loadingLabel="Loading…">Load more</Button></div>:null}
...
function ActivityRow({row,onOpen}:{row:Activity;onOpen:()=>void}) {
  ...
  return <div className="grid gap-3 px-4 py-3 transition hover:bg-slate-50/70 dark:hover:bg-white/[.025] sm:grid-cols-[88px_1fr_auto] sm:items-center"><div><Badge tone="neutral">{chain}</Badge></div><div className="min-w-0"><div className="truncate text-sm font-medium">{title}</div><div className="mt-0.5 flex min-w-0 items-center gap-2"><span className="truncate font-mono text-xs text-slate-500">{short(id)}</span>{status?<span className="shrink-0 text-[11px] font-semibold text-slate-500">{status}</span>:null}</div></div><Button size="sm" onClick={onOpen}>Details</Button></div>;
}

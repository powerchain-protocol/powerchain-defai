...
export const metadata: Metadata = {
  title: "History",
  description: "Review persisted PowerChain bridge operations, recovery states and canonical transfer-status links.",
};
...
export default async function HistoryPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-5">
      ...
      <form method="get" className="pc-theme-card grid gap-2 p-4 sm:grid-cols-[1fr_220px_auto]" role="search">
        <label className="sr-only" htmlFor="history-address">Wallet address</label>
        <Input id="history-address" name="address" defaultValue={address} placeholder="Wallet address" autoComplete="off" spellCheck={false} maxLength={256} />
      ...
      <Card className="overflow-hidden" aria-label="Bridge transaction history">
        {rows.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100 dark:divide-white/10">
            {rows.map((row) => (
              <Link key={row.id} href={bridgeStatusRoute(row.id)} className="grid gap-3 p-4 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#35584a] sm:grid-cols-[140px_minmax(0,1fr)_190px_150px] sm:items-center dark:hover:bg-white/[0.035]">
...
function SummaryCard({ label, value, detail, tone = "default" }: { label: string; value: number; detail: string; tone?: "default" | "attention" }) {
  return <Card className={`p-4 ${tone === "attention" ? "border-rose-200 bg-rose-50/70 dark:border-rose-500/20 dark:bg-rose-500/[0.08]" : ""}`}><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">{label}</p><p className="mt-2 text-2xl font-semibold tabular-nums text-slate-950 dark:text-white">{value}</p><p className="mt-1 text-xs text-slate-500">{detail}</p></Card>;
}

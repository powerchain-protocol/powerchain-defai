...
export async function GET(_request: Request, context: { params: Promise<{ signature: string }> }) {
  try {
    ...
    return NextResponse.json({ code: "SOLANA_TRANSACTION_UNAVAILABLE", message: "Solana transaction details are temporarily unavailable" }, { status: 503, headers: { "Cache-Control": "no-store, max-age=0", Pragma: "no-cache" } });
  }
}

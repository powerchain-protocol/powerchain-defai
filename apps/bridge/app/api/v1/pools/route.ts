...
export async function GET(req:Request){const id=requestId(req);try{const url=new URL(req.url);const chainRaw=url.searchParams.get("chain")?.toUpperCase();const chain=chainRaw==="SOLANA"||chainRaw==="SUI"?chainRaw:undefined;const providerRaw=url.searchParams.get("provider")?.toLowerCase();const provider=providerRaw&&providers.has(providerRaw as DexProvider)?providerRaw as DexProvider:undefined;return ok(await fetchTrustedPools({chain,provider}),200,id,{"Cache-Control":"no-store"});}catch{return fail("POOLS_UNAVAILABLE","DEX pool discovery is temporarily unavailable",503,id,true);}}
